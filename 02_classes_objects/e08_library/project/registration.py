from project.user import User
from project.library import Library


class Registration:

    def add_user(self, user: User, library: Library):
        if user in library.user_records:
            return f'User with id = {user.user_id} already registered in the library!'

        library.user_records.append(user)

    def remove_user(self, user: User, library: Library):
        if user not in library.user_records:
            return 'We could not find such user to remove!'

        library.user_records.remove(user)

    def change_username(self, user_id: int, new_username: str, library: Library):
        try:
            user_to_change = list(filter(lambda x: x.user_id == user_id, library.user_records))[0]
            library.user_records.remove(user_to_change)

            if user_to_change.username == new_username:
                library.user_records.append(user_to_change)
                return f'Please check again the provided username - ' \
                       f'it should be different than the username used so far!'

            if user_to_change.username in library.rented_books.keys():
                library.rented_books[new_username] = library.rented_books.pop(user_to_change.username)

            user_to_change.username = new_username
            library.user_records.append(user_to_change)
            return f'Username successfully changed to: {new_username} for user id: {user_id}'
        except IndexError:
            return f'There is no user with id = {user_id}!'
